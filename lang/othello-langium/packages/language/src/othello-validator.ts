 ...
 */
export function registerValidationChecks(services: OthelloServices) {
    const registry = services.validation.ValidationRegistry;
    const validator = services.validation.OthelloValidator;
    const checks: ValidationChecks<OthelloAstType> = {
...
export class OthelloValidator {
    ...
    checkMoveValidity(game: Game, accept: ValidationAcceptor): void {
        ...
        const occupiedPositions = new Set(
            game.initial?.cells.map(c => `${c.position.row},${c.position.column}`) || []
        );
